...
class Graph:
	...
	x_length : tuple[float,float]
		x-axis pixel length of graph on screen

	y_length : tuple[float,float]
		y-axes pixel length of graph on screen
	...
	def animate(self,canvas,steps):
		...
		for i in range(steps + 1):
			canvas.delete("all")
			canvas.create_line(cx_min,oy,cx_min + self.x_length * i / steps,oy,width = 3)			# x-axis 
			canvas.create_line(ox,cy_min,ox,cy_min - self.y_length * i / steps,width = 3)			# y-axis 
