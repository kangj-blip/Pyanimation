...
from objects.geometry import Line, Arc, Circle, SemiCircle, Polygon, Rect, Square, EqTriangle
...
engine.particleAnimate(line,1000)

# How to make a Circle (make ang negative to make the arc go clockwise)
circle = Circle(r = 300,cen = (750,430),start_ang = math.pi / 2,color = DeltColor,fill = DeltColor)
engine.animate(circle,steps = 30)

# How to move objects
engine.move(circle,delta_x = 20,delta_y = -20,steps = 25)

# How to make a Rectangle
rect = Rect(x = 100,y = 50,width = 200,height = 100,color = DeltColor,fill = DeltColor)
engine.animate(rect,steps = 20)

# How to make a Square
sq = Square(x = 100,width = 50,color = DeltColor,fill = DeltColor)
engine.animate(sq,steps = 20)

$ How to make an Equilateral Triangle
tri = EqTriangle(600,440,200,200,DeltColor,DeltColor)
engine.animate(tri,30)
